...
class NumpyDataset(BaseDataset):
    def __init__(
        ...
        if self.resize:
            self.target = self._resize_data(self.target)
            self.source = self._resize_data(self.source)
    ...
    def _load_data2(self, contrast):
        ...
        data = []
        for file in files:
            data.append(cv2.imread(os.path.join(data_dir, file)))
